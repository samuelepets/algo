...
└── cat01_trend_following/          # one sub-experiment per CATEGORIES.md category
    ...
    │   └── src/                    # Rust source (added during implementation)
    ├── tf02_triple_ema/
    │   └── ...
    └── ... (tf03 – tf20)
```
...
All sub-sub-experiments are implemented as independent **Rust** binary projects
(`cargo init`). Rust is chosen for:

- **Performance:** A full brute-force search over thousands of parameter
  combinations × 22 years of 1-min bars (≈ 5.8 M bars) must complete in
  minutes, not hours. Rust's zero-cost abstractions and cache-friendly data
  structures make this feasible.
- **Reproducibility:** Deterministic execution; no GIL, no interpreter state.
- **Self-contained:** Each Rust project compiles to a single binary with
...
| Category | Status |
|---|---|
| Cat 1 — Trend-Following (20 strategies) | Structure defined; implementation pending |
