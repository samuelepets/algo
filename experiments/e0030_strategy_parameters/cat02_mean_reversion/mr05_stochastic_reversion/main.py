...
from indicators import ATR_PERIOD, D_PERIODS, K_PERIODS, SLOWINGS, atr, build_stoch_cache

OVERSOLD_THRESHOLDS = [15, 20, 25]
OVERBOUGHT_THRESHOLDS = [85, 80, 75]
ATR_STOP_MULTS = [0.75, 1.0, 1.5]
MAX_HOLD_BARS_LIST = [5, 10, 15, 20]
TIMEFRAMES = [5, 15]
...
def _grid_arrays(
    ...
    for i, p in enumerate(grid):
        combo_rows[i] = combo_index[(p.k_period, p.slowing, p.d_period)]
        ...
        warm_bars[i] = p.k_period + p.slowing + p.d_period + 2
    return (
        combo_rows,
        oversold_thresh,
        overbought_thresh,
...
def _prepare_tf(bars_1min: BarArrays, tf_min: int) -> TfData:
    ...
    k_matrix, d_matrix, combo_index = build_stoch_cache(
        bars_tf.high, bars_tf.low, bars_tf.close, K_PERIODS, SLOWINGS, D_PERIODS
    )
    atr_vals = atr(bars_tf.high, bars_tf.low, bars_tf.close, ATR_PERIOD)
    grid = build_grid(tf_min)
    combo_rows, oversold_thresh, overbought_thresh, atr_stop_mult, max_hold_bars, warm_bars = (
        _grid_arrays(grid, combo_index)
    )
    return TfData(
        tf_min=tf_min,
        bars=bars_tf,
...
def _eval_single(tf: TfData, params: Params, start_ts: int, end_ts: int) -> Metrics:
    ...
    warm = params.k_period + params.slowing + params.d_period + 2
    row = tf.combo_index[(params.k_period, params.slowing, params.d_period)]
...
def main() -> None:
    ...
    with results_path.open("w", encoding="utf-8") as fh:
        fh.write(
            "k_period,d_period,slowing,oversold_thresh,overbought_thresh,"
            "atr_stop_mult,max_hold_bars,tf_min,"
            "sharpe,profit_factor,max_drawdown_r,total_return,n_trades\n"
        )
        for p, m in all_results:
            fh.write(
                f"{p.k_period},{p.d_period},{p.slowing},"
                f"{p.oversold_thresh:.1f},{p.overbought_thresh:.1f},"
                f"{p.atr_stop_mult:.2f},{p.max_hold_bars},{p.tf_min},"
    ...
    print(
        f"{'k':<4} {'d':<4} {'slow':<5} {'os':<4} {'ob':<4} {'stop':<5} {'hold':<5} {'tf':<4}  "
        f"{'sharpe':>8}  {'pf':>7}  {'max_dd_r':>8}  {'tot_R':>8}  {'trades':>7}"
    )
    for p, m in valid[:10]:
        print(
            f"{p.k_period:<4} {p.d_period:<4} {p.slowing:<5} "
            f"{p.oversold_thresh:<4.0f} {p.overbought_thresh:<4.0f} "
            f"{p.atr_stop_mult:<5.2f} {p.max_hold_bars:<5} {p.tf_min:<4}  "
    ...
    with wf_path.open("w", encoding="utf-8") as wf:
        wf.write(
            ...
            "sel_k_period,sel_d_period,sel_slowing,sel_oversold_thresh,"
            "sel_overbought_thresh,sel_atr_stop_mult,sel_max_hold_bars,sel_tf,"
        ...
        for idx, (is_s, is_e, oos_s, oos_e) in enumerate(WALK_FORWARD_WINDOWS):
            ...
            selected_label = (
                f"k{sel_params.k_period}/d{sel_params.d_period}/s{sel_params.slowing} "
                f"os{sel_params.oversold_thresh:.0f}/ob{sel_params.overbought_thresh:.0f} "
                f"stop{sel_params.atr_stop_mult:.2f}x "
            ...
            wf.write(
                ...
                f"{sel_params.k_period},{sel_params.d_period},{sel_params.slowing},"
                f"{sel_params.oversold_thresh:.1f},{sel_params.overbought_thresh:.1f},"
                f"{sel_params.atr_stop_mult:.2f},"
