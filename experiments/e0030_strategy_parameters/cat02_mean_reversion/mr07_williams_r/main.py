...
from indicators import ATR_PERIOD, WR_PERIODS, atr, build_wr_cache

OVERSOLD_THRESHOLDS = [-80.0, -85.0, -90.0]
OVERBOUGHT_THRESHOLDS = [-20.0, -15.0, -10.0]
EXIT_LEVELS = [-50.0, -40.0, -60.0]
ATR_STOP_MULTS = [0.75, 1.0, 1.5]
MAX_HOLD_BARS_LIST = [5, 10, 15]
TIMEFRAMES = [1, 5]
...
def _grid_arrays(
    ...
    for i, p in enumerate(grid):
        period_rows[i] = period_index[p.wr_period]
        ...
        warm_bars[i] = max(p.wr_period, ATR_PERIOD) + 1
    return (
        period_rows,
        oversold_thresh,
        overbought_thresh,
...
def _prepare_tf(bars_1min: BarArrays, tf_min: int) -> TfData:
    ...
    wr_matrix, period_index = build_wr_cache(bars_tf.high, bars_tf.low, bars_tf.close, WR_PERIODS)
...
def _eval_single(tf: TfData, params: Params, start_ts: int, end_ts: int) -> Metrics:
    ...
    warm = max(params.wr_period, ATR_PERIOD) + 1
    ...
    search_range(
        ...
        tf.wr_matrix,
        tf.atr_vals,
        np.array([tf.period_index[params.wr_period]], dtype=np.int64),
...
def main() -> None:
    ...
    with results_path.open("w", encoding="utf-8") as fh:
        fh.write(
            "wr_period,oversold_thresh,overbought_thresh,exit_level,atr_stop_mult,"
            "max_hold_bars,tf_min,"
            "sharpe,profit_factor,max_drawdown_r,total_return,n_trades\n"
        )
        for p, m in all_results:
            fh.write(
                f"{p.wr_period},{p.oversold_thresh:.1f},{p.overbought_thresh:.1f},"
                f"{p.exit_level:.1f},{p.atr_stop_mult:.2f},{p.max_hold_bars},{p.tf_min},"
    ...
    print(
        f"{'per':<4} {'oversold':<9} {'overbought':<11} {'exit':<6} {'stop':<5} "
        f"{'hold':<5} {'tf':<4}  "
        f"{'sharpe':>8}  {'pf':>7}  {'max_dd_r':>8}  {'tot_R':>8}  {'trades':>7}"
    )
    for p, m in valid[:10]:
        print(
            f"{p.wr_period:<4} {p.oversold_thresh:<9.1f} {p.overbought_thresh:<11.1f} "
            f"{p.exit_level:<6.1f} {p.atr_stop_mult:<5.2f} {p.max_hold_bars:<5} {p.tf_min:<4}  "
    ...
    with wf_path.open("w", encoding="utf-8") as wf:
        wf.write(
            ...
            "sel_wr_period,sel_oversold_thresh,sel_overbought_thresh,sel_exit_level,"
            "sel_atr_stop_mult,sel_max_hold_bars,sel_tf,"
        ...
        for idx, (is_s, is_e, oos_s, oos_e) in enumerate(WALK_FORWARD_WINDOWS):
            ...
            selected_label = (
                f"wr{sel_params.wr_period} os{sel_params.oversold_thresh:.0f}/"
                f"ob{sel_params.overbought_thresh:.0f} exit{sel_params.exit_level:.0f} "
                f"stop{sel_params.atr_stop_mult:.2f}x hold{sel_params.max_hold_bars} "
            ...
            wf.write(
                ...
                f"{sel_params.wr_period},{sel_params.oversold_thresh:.1f},"
                f"{sel_params.overbought_thresh:.1f},{sel_params.exit_level:.1f},"
