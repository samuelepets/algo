"""Backtest engine for mr07_williams_r.

Strategy: fade short-term Williams %R extremes. Long entry when %R crosses
below ``oversold_thresh``; exit when %R crosses back above ``exit_level`` or
``max_hold_bars`` bars have elapsed (mirror for shorts with
``overbought_thresh``). Stop-loss is ATR(14)-based and checked before the
target/time exit within the same bar (stop priority).
"""
...
def backtest_core(
    ...
    """Run Williams %R mean-reversion backtest; returns metrics tuple."""
    n = len(close)
    if n < 2:
    ...
    for i in range(warm, n):
        ...
        if in_pos:
            ...
            if is_long:
                if low[i] <= stop:
                    ...
                    not np.isnan(wr)
                    and not np.isnan(wr_prev)
                    and wr_prev <= exit_level
                    and wr > exit_level
                ):
        ...
        if np.isnan(wr) or np.isnan(wr_prev) or np.isnan(a):
            continue

        bull_sig = wr_prev >= oversold_thresh and wr < oversold_thresh
        bear_sig = wr_prev <= overbought_thresh and wr > overbought_thresh
