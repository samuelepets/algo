...
from backtest import EXIT_EMA, EXIT_HALF_DISTANCE, FORCED_EXIT_BARS, search_range
...
EXIT_TARGETS = [EXIT_EMA, EXIT_HALF_DISTANCE]
ADX_FILTERS = [0, 20, 25]       # 0 = no filter
ATR_STOP_MULTS = [1.0, 1.5, 2.0]
TIMEFRAMES = [1, 5, 15]

_EXIT_LABELS = {EXIT_EMA: "ema", EXIT_HALF_DISTANCE: "half_distance"}
...
def _grid_arrays(
    ...
    exit_target = np.empty(n, dtype=np.int64)
    ...
    for i, p in enumerate(grid):
        ...
        exit_target[i] = p.exit_target
        adx_filter[i] = p.adx_filter
        atr_stop_mult[i] = p.atr_stop_mult
        warm_bars[i] = max(p.ema_period, p.atr_period, ADX_PERIOD * 2) + 1
    return (
        ema_rows,
        atr_rows,
        distance_thresh,
...
def _eval_single(tf: TfData, params: Params, start_ts: int, end_ts: int) -> Metrics:
    ...
    warm = max(params.ema_period, params.atr_period, ADX_PERIOD * 2) + 1
    ...
    search_range(
        ...
        np.array([params.exit_target], dtype=np.int64),
...
def main() -> None:
    ...
    print(f"  Forced-exit safety net: {FORCED_EXIT_BARS} bars (no max_hold_bars in grid)")
    ...
    with results_path.open("w", encoding="utf-8") as fh:
        fh.write(
            "ema_period,atr_period,distance_thresh,exit_target,adx_filter,atr_stop_mult,tf_min,"
            "sharpe,profit_factor,max_drawdown_r,total_return,n_trades\n"
        )
        for p, m in all_results:
            fh.write(
                f"{p.ema_period},{p.atr_period},{p.distance_thresh:.2f},"
                f"{_EXIT_LABELS[p.exit_target]},{p.adx_filter},{p.atr_stop_mult:.2f},{p.tf_min},"
    ...
    print(
        f"{'ema':<4} {'atr':<4} {'dist':<5} {'exit':<13} {'adx':<5} {'stop':<5} {'tf':<4}  "
        f"{'sharpe':>8}  {'pf':>7}  {'max_dd_r':>8}  {'tot_R':>8}  {'trades':>7}"
    )
    for p, m in valid[:10]:
        adx_lbl = f"<{p.adx_filter}" if p.adx_filter > 0 else "none"
        print(
            f"{p.ema_period:<4} {p.atr_period:<4} {p.distance_thresh:<5.1f} "
            f"{_EXIT_LABELS[p.exit_target]:<13} {adx_lbl:<5} {p.atr_stop_mult:<5.1f} "
            f"{p.tf_min:<4}  "
    ...
    with wf_path.open("w", encoding="utf-8") as wf:
        wf.write(
            ...
            "sel_ema_period,sel_atr_period,sel_distance_thresh,sel_exit_target,"
            "sel_adx_filter,sel_atr_stop_mult,sel_tf,"
        ...
        for idx, (is_s, is_e, oos_s, oos_e) in enumerate(WALK_FORWARD_WINDOWS):
            ...
            selected_label = (
                ...
                f"d{sel_params.distance_thresh:.1f} {_EXIT_LABELS[sel_params.exit_target]} "
                f"{adx_lbl} stop{sel_params.atr_stop_mult:.1f}x {sel_params.tf_min}m"
            )
            print(
            ...
            wf.write(
                ...
                f"{sel_params.ema_period},{sel_params.atr_period},"
                f"{sel_params.distance_thresh:.2f},{_EXIT_LABELS[sel_params.exit_target]},"
