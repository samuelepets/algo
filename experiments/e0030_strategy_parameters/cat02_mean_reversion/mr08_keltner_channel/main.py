...
from backtest import ENTRY_CLOSE_OUTSIDE, ENTRY_WICK_TOUCH, MAX_HOLD_BARS, search_range
from indicators import ATR_PERIODS, EMA_PERIODS, build_atr_cache, build_ema_cache

K_MULTS = [1.0, 1.5, 2.0, 2.5, 3.0]
ENTRY_TYPES = [ENTRY_CLOSE_OUTSIDE, ENTRY_WICK_TOUCH]
ATR_STOP_MULTS = [0.5, 1.0, 1.5]
TIMEFRAMES = [5, 15]

_ENTRY_LABELS = {ENTRY_CLOSE_OUTSIDE: "close_outside", ENTRY_WICK_TOUCH: "wick_touch"}
...
def _grid_arrays(
    grid: list[Params], ema_index: dict[int, int], atr_index: dict[int, int]
) -> tuple[np.ndarray, ...]:
    n = len(grid)
    ema_rows = np.empty(n, dtype=np.int64)
    atr_rows = np.empty(n, dtype=np.int64)
    k_mult = np.empty(n, dtype=np.float64)
    ...
    for i, p in enumerate(grid):
        ...
        warm_bars[i] = max(p.ema_period, p.atr_period) + 1
    return ema_rows, atr_rows, k_mult, entry_type, atr_stop_mult, max_hold_bars, warm_bars
...
def _prepare_tf(bars_1min: BarArrays, tf_min: int) -> TfData:
    ...
    ema_rows, atr_rows, k_mult, entry_type, atr_stop_mult, max_hold_bars, warm_bars = (
        _grid_arrays(grid, ema_index, atr_index)
    )
    return TfData(
        tf_min=tf_min,
        bars=bars_tf,
...
def _eval_single(tf: TfData, params: Params, start_ts: int, end_ts: int) -> Metrics:
    ...
    warm = max(params.ema_period, params.atr_period) + 1
    ...
    search_range(
        ...
        np.array([params.k_mult], dtype=np.float64),
        np.array([params.entry_type], dtype=np.int64),
...
def main() -> None:
    ...
    with results_path.open("w", encoding="utf-8") as fh:
        fh.write(
            "ema_period,atr_period,k_mult,entry_type,atr_stop_mult,tf_min,"
            "sharpe,profit_factor,max_drawdown_r,total_return,n_trades\n"
        )
        for p, m in all_results:
            fh.write(
                f"{p.ema_period},{p.atr_period},{p.k_mult:.1f},{_ENTRY_LABELS[p.entry_type]},"
    ...
    print(
        f"{'ema':<4} {'atr':<4} {'k':<5} {'entry':<14} {'stop':<5} {'tf':<4}  "
        f"{'sharpe':>8}  {'pf':>7}  {'max_dd_r':>8}  {'tot_R':>8}  {'trades':>7}"
    )
    for p, m in valid[:10]:
        print(
            f"{p.ema_period:<4} {p.atr_period:<4} {p.k_mult:<5.1f} "
            f"{_ENTRY_LABELS[p.entry_type]:<14} "
    ...
    with wf_path.open("w", encoding="utf-8") as wf:
        wf.write(
            ...
            "sel_ema_period,sel_atr_period,sel_k_mult,sel_entry_type,"
        ...
        for idx, (is_s, is_e, oos_s, oos_e) in enumerate(WALK_FORWARD_WINDOWS):
            ...
            entry_lbl = _ENTRY_LABELS[sel_params.entry_type]
            selected_label = (
                f"ema{sel_params.ema_period}/atr{sel_params.atr_period} "
                f"k{sel_params.k_mult:.1f}x {entry_lbl} "
            ...
            wf.write(
                ...
                f"{sel_params.ema_period},{sel_params.atr_period},{sel_params.k_mult:.1f},"
                f"{entry_lbl},{sel_params.atr_stop_mult:.2f},{sel_params.tf_min},"
