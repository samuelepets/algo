...
from indicators import ATR_PERIOD, DEMARKER_PERIODS, atr, build_demarker_cache
...
def build_grid(tf_min: int) -> list[Params]:
    grid: list[Params] = []
    for period in DEMARKER_PERIODS:
...
def _grid_arrays(
    ...
    for i, p in enumerate(grid):
        period_rows[i] = period_index[p.demarker_period]
        ...
        warm_bars[i] = p.demarker_period + 2
    return period_rows, oversold_thresh, overbought_thresh, exit_level, atr_stop_mult, warm_bars


def _prepare_tf(bars_1min: BarArrays, tf_min: int) -> TfData:
    bars_tf = resample(bars_1min, tf_min)
    dem_matrix, period_index = build_demarker_cache(bars_tf.high, bars_tf.low, DEMARKER_PERIODS)
    atr_vals = atr(bars_tf.high, bars_tf.low, bars_tf.close, ATR_PERIOD)
    grid = build_grid(tf_min)
    period_rows, oversold_thresh, overbought_thresh, exit_level, atr_stop_mult, warm_bars = (
...
def _eval_single(tf: TfData, params: Params, start_ts: int, end_ts: int) -> Metrics:
    ...
    search_range(
        ...
        tf.dem_matrix,
        tf.atr_vals,
        np.array([tf.period_index[params.demarker_period]], dtype=np.int64),
...
def main() -> None:
    ...
    with results_path.open("w", encoding="utf-8") as fh:
        fh.write(
            "demarker_period,oversold_thresh,overbought_thresh,exit_level,"
        ...
        for p, m in all_results:
            fh.write(
                f"{p.demarker_period},"
                f"{p.oversold_thresh:.2f},{p.overbought_thresh:.2f},{p.exit_level:.2f},"
    ...
    for p, m in valid[:10]:
        print(
            f"{p.demarker_period:<4} "
            f"{p.oversold_thresh:<5.2f} {p.overbought_thresh:<5.2f} {p.exit_level:<5.2f} "
    ...
    with wf_path.open("w", encoding="utf-8") as wf:
        ...
        for idx, (is_s, is_e, oos_s, oos_e) in enumerate(WALK_FORWARD_WINDOWS):
            ...
            selected_label = (
                f"dem{sel_params.demarker_period} "
                f"os{sel_params.oversold_thresh:.2f}/ob{sel_params.overbought_thresh:.2f} "
                f"x{sel_params.exit_level:.2f} stop{sel_params.atr_stop_mult:.2f}x {sel_params.tf_min}m"
            )
            print(
                f"{idx + 1:<4}  {is_s}-{is_e:<7}  {oos_s}-{oos_e:<7}  "
                f"{selected_label:<44}  {is_m.sharpe:>8.4f}  {oos_m.sharpe:>8.4f}  "
                f"{oos_m.profit_factor:>7.4f}  {oos_m.n_trades:>7}"
            )
            wf.write(
                f"{idx + 1},{is_s},{is_e},{oos_s},{oos_e},"
                f"{sel_params.demarker_period},"
                f"{sel_params.oversold_thresh:.2f},{sel_params.overbought_thresh:.2f},"
                f"{sel_params.exit_level:.2f},{sel_params.atr_stop_mult:.2f},{sel_params.tf_min},"
