...
from indicators import ATR_PERIOD, MFI_PERIODS, atr, build_mfi_cache

OVERSOLD_THRESHOLDS = [10.0, 15.0, 20.0, 25.0]
OVERBOUGHT_THRESHOLDS = [90.0, 85.0, 80.0, 75.0]
EXIT_LEVELS = [50.0]
ATR_STOP_MULTS = [1.0, 1.5]
TIMEFRAMES = [5, 15]
...
def build_grid(tf_min: int) -> list[Params]:
    ...
    for period in MFI_PERIODS:
        for os_t in OVERSOLD_THRESHOLDS:
            for ob_t in OVERBOUGHT_THRESHOLDS:
                for x_lvl in EXIT_LEVELS:
                    for atr_m in ATR_STOP_MULTS:
                        grid.append(
                            Params(
                                mfi_period=period,
                                oversold_thresh=os_t,
                                overbought_thresh=ob_t,
                                exit_level=x_lvl,
                                atr_stop_mult=atr_m,
                                tf_min=tf_min,
                            )
...
def _grid_arrays(
    ...
    for i, p in enumerate(grid):
        period_rows[i] = period_index[p.mfi_period]
        ...
        warm_bars[i] = max(p.mfi_period, ATR_PERIOD) + 1
    return (
        period_rows,
        oversold_thresh,
        overbought_thresh,
...
def _prepare_tf(bars_1min: BarArrays, tf_min: int) -> TfData:
    ...
    mfi_matrix, period_index = build_mfi_cache(
        bars_tf.high, bars_tf.low, bars_tf.close, bars_tf.volume, MFI_PERIODS
    )
...
def _eval_single(tf: TfData, params: Params, start_ts: int, end_ts: int) -> Metrics:
    ...
    warm = max(params.mfi_period, ATR_PERIOD) + 1
    ...
    search_range(
        ...
        tf.mfi_matrix,
        tf.atr_vals,
        np.array([tf.period_index[params.mfi_period]], dtype=np.int64),
...
def main() -> None:
    ...
    with results_path.open("w", encoding="utf-8") as fh:
        fh.write(
            "mfi_period,oversold_thresh,overbought_thresh,exit_level,atr_stop_mult,tf_min,"
            "sharpe,profit_factor,max_drawdown_r,total_return,n_trades\n"
        )
        for p, m in all_results:
            fh.write(
                f"{p.mfi_period},{p.oversold_thresh:.1f},{p.overbought_thresh:.1f},"
                f"{p.exit_level:.1f},{p.atr_stop_mult:.2f},{p.tf_min},"
    ...
    print(
        f"{'per':<4} {'os':<5} {'ob':<5} {'xlvl':<5} {'stop':<5} {'tf':<4}  "
        f"{'sharpe':>8}  {'pf':>7}  {'max_dd_r':>8}  {'tot_R':>8}  {'trades':>7}"
    )
    for p, m in valid[:10]:
        print(
            f"{p.mfi_period:<4} {p.oversold_thresh:<5.0f} {p.overbought_thresh:<5.0f} "
            f"{p.exit_level:<5.0f} {p.atr_stop_mult:<5.1f} {p.tf_min:<4}  "
    ...
    with wf_path.open("w", encoding="utf-8") as wf:
        wf.write(
            ...
            "sel_mfi_period,sel_oversold_thresh,sel_overbought_thresh,"
            "sel_exit_level,sel_atr_stop_mult,sel_tf,"
        ...
        for idx, (is_s, is_e, oos_s, oos_e) in enumerate(WALK_FORWARD_WINDOWS):
            ...
            selected_label = (
                f"mfi{sel_params.mfi_period} os{sel_params.oversold_thresh:.0f} "
                f"ob{sel_params.overbought_thresh:.0f} x{sel_params.exit_level:.0f} "
            ...
            wf.write(
                ...
                f"{sel_params.mfi_period},{sel_params.oversold_thresh:.1f},"
