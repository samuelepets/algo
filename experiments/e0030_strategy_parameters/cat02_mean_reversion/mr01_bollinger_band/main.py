...
from backtest import ENTRY_CLOSE_OUTSIDE, ENTRY_REENTRY, search_range
from indicators import ADX_PERIOD, ATR_PERIOD, BB_PERIODS, atr, adx, build_bb_cache

BB_MULTS = [1.0, 1.5, 2.0, 2.5, 3.0]
ENTRY_TYPES = [ENTRY_CLOSE_OUTSIDE, ENTRY_REENTRY]
ADX_FILTERS = [0, 20, 25]       # 0 = no filter
ATR_STOP_MULTS = [0.5, 1.0, 1.5]
MAX_HOLD_BARS_LIST = [5, 10, 20, 30]
TIMEFRAMES = [5, 15]

_ENTRY_LABELS = {ENTRY_CLOSE_OUTSIDE: "close_outside", ENTRY_REENTRY: "reentry"}
...
def _grid_arrays(
    ...
    bb_mult = np.empty(n, dtype=np.float64)
    ...
    return period_rows, bb_mult, entry_type, adx_filter, atr_stop_mult, max_hold_bars, warm_bars
...
def _prepare_tf(bars_1min: BarArrays, tf_min: int) -> TfData:
    ...
    period_rows, bb_mult, entry_type, adx_filter, atr_stop_mult, max_hold_bars, warm_bars = (
...
def _eval_single(tf: TfData, params: Params, start_ts: int, end_ts: int) -> Metrics:
    ...
    search_range(
        ...
        np.array([params.bb_mult], dtype=np.float64),
...
def main() -> None:
    ...
    with results_path.open("w", encoding="utf-8") as fh:
        fh.write(
            "bb_period,bb_mult,entry_type,adx_filter,atr_stop_mult,max_hold_bars,tf_min,"
            "sharpe,profit_factor,max_drawdown_r,total_return,n_trades\n"
        )
        for p, m in all_results:
            fh.write(
                f"{p.bb_period},{p.bb_mult:.1f},{_ENTRY_LABELS[p.entry_type]},"
                f"{p.adx_filter},{p.atr_stop_mult:.2f},{p.max_hold_bars},{p.tf_min},"
    ...
    print(
        f"{'per':<4} {'mult':<5} {'entry':<14} {'adx':<5} {'stop':<5} {'hold':<5} {'tf':<4}  "
        f"{'sharpe':>8}  {'pf':>7}  {'max_dd_r':>8}  {'tot_R':>8}  {'trades':>7}"
    )
    for p, m in valid[:10]:
        adx_lbl = f"<{p.adx_filter}" if p.adx_filter > 0 else "none"
        print(
            f"{p.bb_period:<4} {p.bb_mult:<5.1f} {_ENTRY_LABELS[p.entry_type]:<14} "
            f"{adx_lbl:<5} {p.atr_stop_mult:<5.1f} {p.max_hold_bars:<5} {p.tf_min:<4}  "
    ...
    with wf_path.open("w", encoding="utf-8") as wf:
        wf.write(
            ...
            "sel_bb_period,sel_bb_mult,sel_entry_type,sel_adx_filter,"
        ...
        for idx, (is_s, is_e, oos_s, oos_e) in enumerate(WALK_FORWARD_WINDOWS):
            ...
            selected_label = (
                f"bb{sel_params.bb_period}/{sel_params.bb_mult:.1f}x "
                f"{entry_lbl} {adx_lbl} "
                f"stop{sel_params.atr_stop_mult:.1f}x "
            ...
            wf.write(
                ...
                f"{sel_params.bb_period},{sel_params.bb_mult:.1f},{entry_lbl},"
                f"{sel_params.adx_filter},{sel_params.atr_stop_mult:.2f},"
