...
The Alligator periods/shifts are parametrised by a single global scale factor
applied to the canonical Fibonacci set (Jaw 13/8, Teeth 8/5, Lips 5/3), as
recommended in the strategy spec.
"""
...
from indicators import ATR_PERIOD, atr, shifted, smma
...
# Canonical Fibonacci Alligator (jaw, teeth, lips).
BASE_PERIODS = (13, 8, 5)
BASE_SHIFTS = (8, 5, 3)
...
def _scaled(scale: float) -> tuple[int, int, int, int, int, int]:
    """Return (jaw_p, teeth_p, lips_p, jaw_s, teeth_s, lips_s) for a scale factor."""
    jp = max(1, int(round(BASE_PERIODS[0] * scale)))
    tp = max(1, int(round(BASE_PERIODS[1] * scale)))
    lp = max(1, int(round(BASE_PERIODS[2] * scale)))
    js = max(0, int(round(BASE_SHIFTS[0] * scale)))
    tsh = max(0, int(round(BASE_SHIFTS[1] * scale)))
    ls = max(0, int(round(BASE_SHIFTS[2] * scale)))
    return jp, tp, lp, js, tsh, ls
...
def build_grid(tf_min: int) -> list[Params]:
    ...
    for scale in SCALE_FACTORS:
        for atr_m in ATR_MULTS:
            grid.append(Params(scale_factor=scale, atr_stop_mult=atr_m, tf_min=tf_min))
    return grid


def _build_lines(bars: BarArrays) -> tuple[np.ndarray, np.ndarray, np.ndarray, dict[float, int], dict[float, int]]:
    median = (bars.high + bars.low) / 2.0
    n = bars.close.size
    jaw_m = np.empty((len(SCALE_FACTORS), n), dtype=np.float64)
    teeth_m = np.empty((len(SCALE_FACTORS), n), dtype=np.float64)
    lips_m = np.empty((len(SCALE_FACTORS), n), dtype=np.float64)
    scale_to_row: dict[float, int] = {}
    warm_for_scale: dict[float, int] = {}
    for row, scale in enumerate(SCALE_FACTORS):
        jp, tp, lp, js, tsh, ls = _scaled(scale)
        jaw_m[row] = shifted(smma(median, jp), js)
        teeth_m[row] = shifted(smma(median, tp), tsh)
        lips_m[row] = shifted(smma(median, lp), ls)
        scale_to_row[scale] = row
        warm_for_scale[scale] = jp + js
    return jaw_m, teeth_m, lips_m, scale_to_row, warm_for_scale


def _grid_arrays(
    grid: list[Params], scale_to_row: dict[float, int], warm_for_scale: dict[float, int]
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    n = len(grid)
    scale_rows = np.empty(n, dtype=np.int64)
    warm_bars = np.empty(n, dtype=np.int64)
    atr_mult = np.empty(n, dtype=np.float64)
    for i, p in enumerate(grid):
        scale_rows[i] = scale_to_row[p.scale_factor]
        warm_bars[i] = warm_for_scale[p.scale_factor]
        atr_mult[i] = p.atr_stop_mult
    return scale_rows, warm_bars, atr_mult
...
def _prepare_tf(bars_1min: BarArrays, tf_min: int) -> TfData:
    ...
    jaw_m, teeth_m, lips_m, scale_to_row, warm_for_scale = _build_lines(bars_tf)
    atr_vals = atr(bars_tf.high, bars_tf.low, bars_tf.close, ATR_PERIOD)
    grid = build_grid(tf_min)
    scale_rows, warm_bars, atr_mult = _grid_arrays(grid, scale_to_row, warm_for_scale)
    return TfData(
        tf_min=tf_min,
        bars=bars_tf,
        jaw_matrix=jaw_m,
        teeth_matrix=teeth_m,
        lips_matrix=lips_m,
        scale_to_row=scale_to_row,
        atr_vals=atr_vals,
        grid=grid,
...
def _eval_single(tf: TfData, params: Params, start_ts: int, end_ts: int) -> Metrics:
    ...
    jp, _tp, _lp, js, _tsh, _ls = _scaled(params.scale_factor)
    scale_rows = np.array([tf.scale_to_row[params.scale_factor]], dtype=np.int64)
    warm_bars = np.array([jp + js], dtype=np.int64)
...
def main() -> None:
    ...
    with results_path.open("w", encoding="utf-8") as fh:
        fh.write(
            "scale_factor,atr_stop_mult,tf_min,"
            "sharpe,profit_factor,max_drawdown_r,total_return,n_trades\n"
        )
        for p, m in all_results:
            fh.write(
                f"{p.scale_factor:.2f},{p.atr_stop_mult:.2f},{p.tf_min},"
    ...
    print(
        f"{'scale':<6} {'atr_m':<6} {'tf':<5}  "
        f"{'sharpe':>8}  {'pf':>8}  {'max_dd_r':>8}  {'tot_R':>8}  {'trades':>7}"
    )
    top10 = []
    for p, m in valid[:10]:
        print(
            f"{p.scale_factor:<6.2f} {p.atr_stop_mult:<6.2f} {p.tf_min:<5}  "
