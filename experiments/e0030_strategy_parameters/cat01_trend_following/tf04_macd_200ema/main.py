...
def build_grid(tf_min: int) -> list[Params]:
    ...
    for trend in TREND_EMAS:
        for fast in MACD_FASTS:
            for slow in MACD_SLOWS:
                if slow <= fast:
                    continue
...
def _grid_arrays(
    grid: list[Params], period_to_row: dict[int, int]
) -> tuple[np.ndarray, ...]:
    n = len(grid)
    ...
    signal_p = np.empty(n, dtype=np.int64)
    atr_mult = np.empty(n, dtype=np.float64)
    for i, p in enumerate(grid):
        fast_rows[i] = period_to_row[p.macd_fast]
        slow_rows[i] = period_to_row[p.macd_slow]
        trend_rows[i] = period_to_row[p.trend_ema]
        slow_p[i] = p.macd_slow
        trend_p[i] = p.trend_ema
        signal_p[i] = p.macd_signal
        atr_mult[i] = p.atr_stop_mult
    return fast_rows, slow_rows, trend_rows, slow_p, trend_p, signal_p, atr_mult
...
def _prepare_tf(bars_1min: BarArrays, tf_min: int) -> TfData:
    ...
    periods = sorted(set(TREND_EMAS) | set(MACD_FASTS) | set(MACD_SLOWS))
    ...
    fast_rows, slow_rows, trend_rows, slow_p, trend_p, signal_p, atr_mult = _grid_arrays(
        grid, period_to_row
    )
    return TfData(
        tf_min=tf_min,
...
def _eval_single(tf: TfData, params: Params, start_ts: int, end_ts: int) -> Metrics:
    ...
    fast_row = np.array([tf.period_to_row[params.macd_fast]], dtype=np.int64)
    slow_row = np.array([tf.period_to_row[params.macd_slow]], dtype=np.int64)
    trend_row = np.array([tf.period_to_row[params.trend_ema]], dtype=np.int64)
    slow_p = np.array([params.macd_slow], dtype=np.int64)
    trend_p = np.array([params.trend_ema], dtype=np.int64)
    signal_p = np.array([params.macd_signal], dtype=np.int64)
...
def main() -> None:
    ...
    with results_path.open("w", encoding="utf-8") as fh:
        fh.write(
            "trend_ema,macd_fast,macd_slow,macd_signal,atr_stop_mult,tf_min,"
            "sharpe,profit_factor,max_drawdown_r,total_return,n_trades\n"
        )
        for p, m in all_results:
            fh.write(
                f"{p.trend_ema},{p.macd_fast},{p.macd_slow},{p.macd_signal},"
    ...
    print(
        f"{'trend':<6} {'fast':<5} {'slow':<5} {'sig':<5} {'atr_m':<6} {'tf':<5}  "
        f"{'sharpe':>8}  {'pf':>8}  {'max_dd_r':>8}  {'tot_R':>8}  {'trades':>7}"
    )
    top10 = []
    for p, m in valid[:10]:
        print(
            f"{p.trend_ema:<6} {p.macd_fast:<5} {p.macd_slow:<5} {p.macd_signal:<5} "
    ...
    with wf_path.open("w", encoding="utf-8") as wf:
        wf.write(
            ...
            "sel_trend,sel_fast,sel_slow,sel_signal,sel_atr_mult,sel_tf,"
        ...
        for idx, (is_s, is_e, oos_s, oos_e) in enumerate(WALK_FORWARD_WINDOWS):
            ...
            selected_label = (
                f"e{sel_params.trend_ema} "
                f"{sel_params.macd_fast}/{sel_params.macd_slow}/{sel_params.macd_signal} "
            ...
            wf.write(
                ...
                f"{sel_params.trend_ema},{sel_params.macd_fast},{sel_params.macd_slow},"
                f"{sel_params.macd_signal},{sel_params.atr_stop_mult:.2f},"
                f"{sel_params.tf_min},"
