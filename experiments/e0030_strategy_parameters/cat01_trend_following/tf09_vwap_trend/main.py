...
from backtest import ENTRY_ABOVE, ENTRY_BOUNCE, search_range
from data import BarArrays, load_eurusd, resample, ts_range, year_start_ts
from indicators import ATR_PERIOD, SESSION_ANCHOR_SECS, atr, build_vwap_cache

os.environ.setdefault("NUMBA_CACHE_DIR", str(Path(__file__).resolve().parent / ".numba_cache"))

# (label, anchor_secs)
VWAP_RESETS = [("daily", 0), ("session", SESSION_ANCHOR_SECS)]
ENTRY_TYPES = [("vwap_bounce", ENTRY_BOUNCE), ("above_vwap", ENTRY_ABOVE)]
...
class TfData:
    ...
    anchor_row: dict[int, int]
    atr_vals: np.ndarray
    grid: list[Params]
    vwap_rows: np.ndarray
...
def build_grid(tf_min: int) -> list[Params]:
    grid: list[Params] = []
    for reset_label, anchor in VWAP_RESETS:
        for entry_label, entry_type in ENTRY_TYPES:
            for sigma in BAND_SIGMAS:
                for atr_m in ATR_MULTS:
                    grid.append(
                        Params(
                            vwap_reset=reset_label,
                            anchor_secs=anchor,
...
def _prepare_tf(bars_1min: BarArrays, tf_min: int) -> TfData:
    ...
    anchors = [a for _label, a in VWAP_RESETS]
    vwap_matrix, std_matrix, anchor_row = build_vwap_cache(
        bars_tf.high, bars_tf.low, bars_tf.close, bars_tf.volume, bars_tf.ts, anchors
    )
    atr_vals = atr(bars_tf.high, bars_tf.low, bars_tf.close, ATR_PERIOD)
    grid = build_grid(tf_min)
    vwap_rows, entry_type, sigma, atr_mult = _grid_arrays(grid, anchor_row)
    return TfData(
        tf_min=tf_min,
        bars=bars_tf,
...
def _eval_single(tf: TfData, params: Params, start_ts: int, end_ts: int) -> Metrics:
    ...
    vwap_rows = np.array([tf.anchor_row[params.anchor_secs]], dtype=np.int64)
    entry_type = np.array([params.entry_type], dtype=np.int64)
    sigma = np.array([params.band_sigma], dtype=np.float64)
...
def main() -> None:
    ...
    with results_path.open("w", encoding="utf-8") as fh:
        fh.write(
            "vwap_reset,entry_type,band_sigma,atr_stop_mult,tf_min,"
            "sharpe,profit_factor,max_drawdown_r,total_return,n_trades\n"
        )
        for p, m in all_results:
            fh.write(
                f"{p.vwap_reset},{p.entry_label},{p.band_sigma:.1f},"
    ...
    print(
        f"{'reset':<8} {'entry':<13} {'sigma':<6} {'atr_m':<6} {'tf':<5}  "
        f"{'sharpe':>8}  {'pf':>8}  {'max_dd_r':>8}  {'tot_R':>8}  {'trades':>7}"
    )
    top10 = []
    for p, m in valid[:10]:
        print(
            f"{p.vwap_reset:<8} {p.entry_label:<13} {p.band_sigma:<6.1f} "
            f"{p.atr_stop_mult:<6.2f} {p.tf_min:<5}  "
    ...
    with wf_path.open("w", encoding="utf-8") as wf:
        wf.write(
            ...
            "sel_vwap_reset,sel_entry_type,sel_band_sigma,sel_atr_mult,sel_tf,"
        ...
        for idx, (is_s, is_e, oos_s, oos_e) in enumerate(WALK_FORWARD_WINDOWS):
            ...
            selected_label = (
                f"{sel_params.vwap_reset} {sel_params.entry_label} "
                f"s{sel_params.band_sigma:.1f} {sel_params.atr_stop_mult:.2f}× "
            ...
            wf.write(
                ...
                f"{sel_params.vwap_reset},{sel_params.entry_label},"
                f"{sel_params.band_sigma:.1f},{sel_params.atr_stop_mult:.2f},"
                f"{sel_params.tf_min},"
