...
FAST_EMAS = [3, 5, 8, 9]
MID_EMAS = [13, 20, 21, 34]
SLOW_EMAS = [34, 50, 55, 89]
# pullback_ema: 0 = mid EMA, 1 = slow EMA
PULLBACK_OPTIONS = [0, 1]
...
def _grid_arrays(
    ...
    mid_rows = np.empty(n, dtype=np.int64)
    slow_rows = np.empty(n, dtype=np.int64)
    pb_rows = np.empty(n, dtype=np.int64)
    slow_p = np.empty(n, dtype=np.int64)
    atr_mult = np.empty(n, dtype=np.float64)
    for i, p in enumerate(grid):
        fast_rows[i] = period_to_row[p.fast_ema]
        mid_rows[i] = period_to_row[p.mid_ema]
        slow_rows[i] = period_to_row[p.slow_ema]
        pb_rows[i] = period_to_row[p.mid_ema] if p.pullback_ema == 0 else period_to_row[p.slow_ema]
        slow_p[i] = p.slow_ema
        atr_mult[i] = p.atr_stop_mult
    return fast_rows, mid_rows, slow_rows, pb_rows, slow_p, atr_mult
...
def _top_params_entry(p: Params, m: Metrics) -> dict[str, float | int | str]:
    return {
        ...
        "pullback_ema": "mid" if p.pullback_ema == 0 else "slow",
        "rr_ratio": RR_RATIO,
        "sharpe": m.sharpe,
        "slow_ema": p.slow_ema,
        "tf_min": p.tf_min,
...
def _prepare_tf(bars_1min: BarArrays, tf_min: int) -> TfData:
    ...
    periods = sorted(set(FAST_EMAS) | set(MID_EMAS) | set(SLOW_EMAS))
    ...
    fast_rows, mid_rows, slow_rows, pb_rows, slow_p, atr_mult = _grid_arrays(grid, period_to_row)
    return TfData(
        tf_min=tf_min,
        bars=bars_tf,
...
def _eval_single(tf: TfData, params: Params, start_ts: int, end_ts: int) -> Metrics:
    ...
    mid_row = np.array([tf.period_to_row[params.mid_ema]], dtype=np.int64)
    slow_row = np.array([tf.period_to_row[params.slow_ema]], dtype=np.int64)
    pb_row = np.array(
        [
            tf.period_to_row[params.mid_ema]
            if params.pullback_ema == 0
            else tf.period_to_row[params.slow_ema]
        ],
        dtype=np.int64,
    )
...
def main() -> None:
    ...
    with results_path.open("w", encoding="utf-8") as fh:
        fh.write(
            "fast_ema,mid_ema,slow_ema,pullback_ema,atr_stop_mult,tf_min,"
            "sharpe,profit_factor,max_drawdown_r,total_return,n_trades\n"
        )
        for p, m in all_results:
            pb_label = "mid" if p.pullback_ema == 0 else "slow"
            fh.write(
                f"{p.fast_ema},{p.mid_ema},{p.slow_ema},{pb_label},"
    ...
    print(
        f"{'fast':<5} {'mid':<5} {'slow':<5} {'pb':<5} {'atr_m':<6} {'tf':<5}  "
    ...
    for p, m in valid[:10]:
        pb_label = "mid" if p.pullback_ema == 0 else "slow"
        print(
            f"{p.fast_ema:<5} {p.mid_ema:<5} {p.slow_ema:<5} {pb_label:<5} "
    ...
    with wf_path.open("w", encoding="utf-8") as wf:
        wf.write(
            ...
            "sel_fast,sel_mid,sel_slow,sel_pb,sel_atr_mult,sel_tf,"
        ...
        for idx, (is_s, is_e, oos_s, oos_e) in enumerate(WALK_FORWARD_WINDOWS):
            ...
            pb_label = "mid" if sel_params.pullback_ema == 0 else "slow"
            selected_label = (
                f"{sel_params.fast_ema}/{sel_params.mid_ema}/{sel_params.slow_ema} "
                f"pb={pb_label} {sel_params.atr_stop_mult:.1f}× "
            ...
            wf.write(
                ...
                f"{sel_params.fast_ema},{sel_params.mid_ema},{sel_params.slow_ema},"
                f"{pb_label},{sel_params.atr_stop_mult:.2f},{sel_params.tf_min},"
