"""Event-driven backtest engine for tf03_macd_crossover.

The hot path is a Numba-compiled state machine implementing the MACD line/signal
crossover strategy: enter long when the MACD line crosses above its signal line
(optionally requiring MACD > 0); exit on stop, target, or the opposite crossover.
The MACD line and signal are reconstructed inside the kernel from precomputed
fast/slow EMAs so the grid search can share one EMA cache.
"""
...
SPREAD = 0.00008
MAX_TRADES = 200_000
RR_RATIO = 2.0  # fixed reward-to-risk; not a grid parameter for TF-03
...
def backtest_core(
    ...
    for i in range(warm, n):
        ...
        if not in_pos and not just_exited:
            atr_v = atr_vals[i]
            long_ok = bull_cross and (zero_filter == 0 or m_now > 0.0)
            short_ok = bear_cross and (zero_filter == 0 or m_now < 0.0)
