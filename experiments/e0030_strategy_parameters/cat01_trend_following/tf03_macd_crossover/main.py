...
FAST_EMAS = [5, 8, 10, 12]
SLOW_EMAS = [21, 24, 26, 30, 35]
SIGNALS = [7, 9, 12]
ZERO_FILTERS = [0, 1]  # 0 = no zero-line filter, 1 = require MACD on zero side
...
def _grid_arrays(
    ...
    zero_filter = np.empty(n, dtype=np.int64)
    ...
    for i, p in enumerate(grid):
        ...
        slow_p[i] = p.slow_ema
        signal_p[i] = p.signal
        zero_filter[i] = p.zero_filter
        atr_mult[i] = p.atr_stop_mult
    return fast_rows, slow_rows, slow_p, signal_p, zero_filter, atr_mult
...
def _top_params_entry(p: Params, m: Metrics) -> dict[str, float | int | str]:
    return {
        ...
        "zero_filter": bool(p.zero_filter),
    }
...
def _prepare_tf(bars_1min: BarArrays, tf_min: int) -> TfData:
    ...
    fast_rows, slow_rows, slow_p, signal_p, zero_filter, atr_mult = _grid_arrays(
        grid, period_to_row
    )
    return TfData(
        tf_min=tf_min,
...
def _eval_single(tf: TfData, params: Params, start_ts: int, end_ts: int) -> Metrics:
    ...
    signal_p = np.array([params.signal], dtype=np.int64)
    zero_filter = np.array([params.zero_filter], dtype=np.int64)
...
def main() -> None:
    ...
    with results_path.open("w", encoding="utf-8") as fh:
        fh.write(
            "fast_ema,slow_ema,signal,zero_filter,atr_stop_mult,tf_min,"
            "sharpe,profit_factor,max_drawdown_r,total_return,n_trades\n"
        )
        for p, m in all_results:
            fh.write(
                f"{p.fast_ema},{p.slow_ema},{p.signal},{bool(p.zero_filter)},"
    ...
    print(
        f"{'fast':<5} {'slow':<5} {'sig':<5} {'zero':<6} {'atr_m':<6} {'tf':<5}  "
        f"{'sharpe':>8}  {'pf':>8}  {'max_dd_r':>8}  {'tot_R':>8}  {'trades':>7}"
    )
    top10 = []
    for p, m in valid[:10]:
        print(
            f"{p.fast_ema:<5} {p.slow_ema:<5} {p.signal:<5} "
            f"{str(bool(p.zero_filter)):<6} {p.atr_stop_mult:<6.1f} {p.tf_min:<5}  "
    ...
    with wf_path.open("w", encoding="utf-8") as wf:
        wf.write(
            ...
            "sel_fast,sel_slow,sel_signal,sel_zero,sel_atr_mult,sel_tf,"
        ...
        for idx, (is_s, is_e, oos_s, oos_e) in enumerate(WALK_FORWARD_WINDOWS):
            ...
            selected_label = (
                f"{sel_params.fast_ema}/{sel_params.slow_ema}/{sel_params.signal} "
                f"z={bool(sel_params.zero_filter)} {sel_params.atr_stop_mult:.1f}× "
            ...
            wf.write(
                ...
                f"{sel_params.fast_ema},{sel_params.slow_ema},{sel_params.signal},"
                f"{bool(sel_params.zero_filter)},{sel_params.atr_stop_mult:.2f},"
                f"{sel_params.tf_min},"
