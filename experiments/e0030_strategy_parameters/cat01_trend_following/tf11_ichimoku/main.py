...
from backtest import RR_RATIO, SIGNAL_CLOUD, SIGNAL_FULL, SIGNAL_TK, search_range
from data import BarArrays, load_eurusd, resample, ts_range, year_start_ts
from indicators import build_midpoint_cache
...
KIJUNS = [20, 22, 26, 30]
SENKOU_BS = [44, 52, 60]
SIGNAL_TYPES = [("full_signal", SIGNAL_FULL), ("tk_cross", SIGNAL_TK), ("cloud_break", SIGNAL_CLOUD)]
TIMEFRAMES = [15, 60]
MIN_TRADES = 50
...
def build_grid(tf_min: int) -> list[Params]:
    ...
    for tenkan in TENKANS:
        for kijun in KIJUNS:
            if tenkan >= kijun:
                continue
            for senkou_b in SENKOU_BS:
                for label, st in SIGNAL_TYPES:
                    grid.append(
                        Params(
                            tenkan=tenkan,
                            kijun=kijun,
...
def _grid_arrays(grid: list[Params], mid_row: dict[int, int]) -> tuple[np.ndarray, ...]:
    n = len(grid)
    tenkan_rows = np.empty(n, dtype=np.int64)
    kijun_rows = np.empty(n, dtype=np.int64)
    senkb_rows = np.empty(n, dtype=np.int64)
    disp = np.empty(n, dtype=np.int64)
    senkou_b_p = np.empty(n, dtype=np.int64)
    signal_type = np.empty(n, dtype=np.int64)
    for i, p in enumerate(grid):
        tenkan_rows[i] = mid_row[p.tenkan]
        kijun_rows[i] = mid_row[p.kijun]
        senkb_rows[i] = mid_row[p.senkou_b]
        disp[i] = p.kijun  # displacement = Kijun period (traditional)
        senkou_b_p[i] = p.senkou_b
        signal_type[i] = p.signal_type
    return tenkan_rows, kijun_rows, senkb_rows, disp, senkou_b_p, signal_type
...
def _prepare_tf(bars_1min: BarArrays, tf_min: int) -> TfData:
    ...
    periods = sorted(set(TENKANS) | set(KIJUNS) | set(SENKOU_BS))
    mid_matrix, mid_row = build_midpoint_cache(bars_tf.high, bars_tf.low, periods)
    grid = build_grid(tf_min)
    tenkan_rows, kijun_rows, senkb_rows, disp, senkou_b_p, signal_type = _grid_arrays(
        grid, mid_row
    )
    return TfData(
        tf_min=tf_min,
        bars=bars_tf,
        mid_matrix=mid_matrix,
        mid_row=mid_row,
        grid=grid,
        tenkan_rows=tenkan_rows,
        kijun_rows=kijun_rows,
        senkb_rows=senkb_rows,
        disp=disp,
...
def _eval_single(tf: TfData, params: Params, start_ts: int, end_ts: int) -> Metrics:
    ...
    tenkan_rows = np.array([tf.mid_row[params.tenkan]], dtype=np.int64)
    kijun_rows = np.array([tf.mid_row[params.kijun]], dtype=np.int64)
    senkb_rows = np.array([tf.mid_row[params.senkou_b]], dtype=np.int64)
    disp = np.array([params.kijun], dtype=np.int64)
    senkou_b_p = np.array([params.senkou_b], dtype=np.int64)
    signal_type = np.array([params.signal_type], dtype=np.int64)
...
def main() -> None:
    ...
    with results_path.open("w", encoding="utf-8") as fh:
        fh.write(
            "tenkan,kijun,senkou_b,signal_type,tf_min,"
            "sharpe,profit_factor,max_drawdown_r,total_return,n_trades\n"
        )
        for p, m in all_results:
            fh.write(
                f"{p.tenkan},{p.kijun},{p.senkou_b},{p.signal_label},{p.tf_min},"
    ...
    print(
        f"{'ten':<5} {'kij':<5} {'senB':<5} {'signal':<12} {'tf':<5}  "
        f"{'sharpe':>8}  {'pf':>8}  {'max_dd_r':>8}  {'tot_R':>8}  {'trades':>7}"
    )
    top10 = []
    for p, m in valid[:10]:
        print(
            f"{p.tenkan:<5} {p.kijun:<5} {p.senkou_b:<5} {p.signal_label:<12} {p.tf_min:<5}  "
    ...
    with wf_path.open("w", encoding="utf-8") as wf:
        wf.write(
            ...
            "sel_tenkan,sel_kijun,sel_senkou_b,sel_signal,sel_tf,"
        ...
        for idx, (is_s, is_e, oos_s, oos_e) in enumerate(WALK_FORWARD_WINDOWS):
            ...
            selected_label = (
                f"{sel_params.tenkan}/{sel_params.kijun}/{sel_params.senkou_b} "
                f"{sel_params.signal_label} {sel_params.tf_min}m"
            )
            print(
            ...
            wf.write(
                ...
                f"{sel_params.tenkan},{sel_params.kijun},{sel_params.senkou_b},"
                f"{sel_params.signal_label},{sel_params.tf_min},"
