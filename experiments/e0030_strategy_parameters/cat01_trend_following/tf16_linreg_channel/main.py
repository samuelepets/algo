"""TF-16 Linear Regression Channel parameter search."""
...
from indicators import ATR_PERIOD, ENTRY_BOUNCE, ENTRY_BREAKOUT, atr, build_linreg_cache

os.environ.setdefault("NUMBA_CACHE_DIR", str(Path(__file__).resolve().parent / ".numba_cache"))

LINREG_PERIODS = [30, 50, 75, 100, 150, 200]
CHANNEL_KS = [1.0, 1.5, 2.0, 2.5]
ENTRY_TYPES = [ENTRY_BOUNCE, ENTRY_BREAKOUT]
...
_ENTRY_LABELS = {ENTRY_BOUNCE: "bounce_center", ENTRY_BREAKOUT: "breakout_upper"}
...
def build_grid(tf_min: int) -> list[Params]:
    ...
    for period in LINREG_PERIODS:
        for ck in CHANNEL_KS:
            for et in ENTRY_TYPES:
                for atr_m in ATR_MULTS:
                    grid.append(
                        Params(
                            linreg_period=period,
                            channel_k=ck,
                            entry_type=et,
                            entry_label=_ENTRY_LABELS[et],
                            atr_stop_mult=atr_m,
                            tf_min=tf_min,
                        )
                    )
...
def _grid_arrays(grid: list[Params], lr_row: dict[int, int]) -> tuple[np.ndarray, ...]:
    n = len(grid)
    lr_rows = np.empty(n, dtype=np.int64)
    warm_bars = np.empty(n, dtype=np.int64)
    entry_type = np.empty(n, dtype=np.int64)
    channel_k = np.empty(n, dtype=np.float64)
    atr_mult = np.empty(n, dtype=np.float64)
    for i, p in enumerate(grid):
        lr_rows[i] = lr_row[p.linreg_period]
        warm_bars[i] = p.linreg_period
        entry_type[i] = p.entry_type
        channel_k[i] = p.channel_k
        atr_mult[i] = p.atr_stop_mult
    return lr_rows, warm_bars, entry_type, channel_k, atr_mult


def _top_params_entry(p: Params, m: Metrics) -> dict[str, float | int | str]:
    return {
        "atr_stop_mult": p.atr_stop_mult,
        "channel_k": p.channel_k,
        "entry_type": p.entry_label,
        "linreg_period": p.linreg_period,
...
def _prepare_tf(bars_1min: BarArrays, tf_min: int) -> TfData:
    ...
    periods = LINREG_PERIODS
    center_matrix, sigma_matrix, lr_row = build_linreg_cache(bars_tf.close, periods)
    atr_vals = atr(bars_tf.high, bars_tf.low, bars_tf.close, ATR_PERIOD)
    grid = build_grid(tf_min)
    lr_rows, warm_bars, entry_type, channel_k, atr_mult = _grid_arrays(grid, lr_row)
    return TfData(
        tf_min=tf_min,
        bars=bars_tf,
...
def _eval_single(tf: TfData, params: Params, start_ts: int, end_ts: int) -> Metrics:
    ...
    lr_rows = np.array([tf.lr_row[params.linreg_period]], dtype=np.int64)
    warm_bars = np.array([params.linreg_period], dtype=np.int64)
    entry_type = np.array([params.entry_type], dtype=np.int64)
    channel_k = np.array([params.channel_k], dtype=np.float64)
...
def main() -> None:
    ...
    with results_path.open("w", encoding="utf-8") as fh:
        fh.write(
            "linreg_period,channel_k,entry_type,atr_stop_mult,tf_min,"
            "sharpe,profit_factor,max_drawdown_r,total_return,n_trades\n"
        )
        for p, m in all_results:
            fh.write(
                f"{p.linreg_period},{p.channel_k:.2f},{p.entry_label},"
    ...
    print(
        f"{'period':<8} {'k':<5} {'entry':<16} {'atr_m':<6} {'tf':<5}  "
        f"{'sharpe':>8}  {'pf':>8}  {'max_dd_r':>8}  {'tot_R':>8}  {'trades':>7}"
    )
    top10 = []
    for p, m in valid[:10]:
        print(
            f"{p.linreg_period:<8} {p.channel_k:<5.1f} {p.entry_label:<16} "
    ...
    with wf_path.open("w", encoding="utf-8") as wf:
        wf.write(
            ...
            "sel_period,sel_k,sel_entry,sel_atr_mult,sel_tf,"
            "is_sharpe,is_pf,is_n_trades,"
            "oos_sharpe,oos_pf,oos_max_drawdown_r,oos_n_trades\n"
        )
        print(
            f"\n{'win':<4}  {'IS':<12}  {'OOS':<12}  {'selected':<36}  "
        ...
        for idx, (is_s, is_e, oos_s, oos_e) in enumerate(WALK_FORWARD_WINDOWS):
            ...
            selected_label = (
                f"lr{sel_params.linreg_period} k{sel_params.channel_k:.1f} "
                f"{sel_params.entry_label} {sel_params.atr_stop_mult:.1f}x {sel_params.tf_min}m"
            )
            print(
                f"{idx + 1:<4}  {is_s}-{is_e:<7}  {oos_s}-{oos_e:<7}  "
                f"{selected_label:<36}  {is_m.sharpe:>8.4f}  {oos_m.sharpe:>8.4f}  "
                f"{oos_m.profit_factor:>8.4f}  {oos_m.n_trades:>7}"
            )
            wf.write(
                f"{idx + 1},{is_s},{is_e},{oos_s},{oos_e},"
                f"{sel_params.linreg_period},{sel_params.channel_k:.2f},"
                f"{sel_params.entry_label},{sel_params.atr_stop_mult:.2f},{sel_params.tf_min},"
