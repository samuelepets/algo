"""Data access layer for tf16_linreg_channel.
...
# experiments/.../tf16_linreg_channel/data.py -> repo root is four levels up.
...
def load_eurusd(data_dir: Path | None = None) -> BarArrays:
    ...
    ts = ts[order]; open_ = open_[order]; high = high[order]
    low = low[order]; close = close[order]; volume = volume[order]
    _, unique_idx = np.unique(ts, return_index=True)
    unique_idx.sort()
    return BarArrays(
        ts=ts[unique_idx], open=open_[unique_idx], high=high[unique_idx],
        low=low[unique_idx], close=close[unique_idx], volume=volume[unique_idx],
    )


@njit(cache=True)
def _resample_core(ts, open_, high, low, close, volume, bucket_secs):
    n = len(ts)
    if n == 0:
    ...
    o = open_[0]; h = high[0]; lo = low[0]; c = close[0]; v = volume[0]
    out_idx = 0
    for i in range(1, n):
        ...
        if b == cur_bucket:
            ...
            c = close[i]; v += volume[i]
        else:
            out_ts[out_idx] = cur_bucket; out_open[out_idx] = o
            out_high[out_idx] = h; out_low[out_idx] = lo
            out_close[out_idx] = c; out_volume[out_idx] = v
            out_idx += 1
            cur_bucket = b; o = open_[i]; h = high[i]; lo = low[i]; c = close[i]; v = volume[i]
    out_ts[out_idx] = cur_bucket; out_open[out_idx] = o; out_high[out_idx] = h
    out_low[out_idx] = lo; out_close[out_idx] = c; out_volume[out_idx] = v
    out_idx += 1
    return out_ts[:out_idx], out_open[:out_idx], out_high[:out_idx], out_low[:out_idx], out_close[:out_idx], out_volume[:out_idx]
