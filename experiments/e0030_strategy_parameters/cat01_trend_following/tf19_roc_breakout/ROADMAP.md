# Roadmap — TF-19 ROC Breakout

## Phase 0 — Rust project setup
- [ ] `cargo init --name tf19_roc_breakout` in this directory.
...
## Phase 2 — Resampling
- [ ] Implement `resample(bars, minutes)` → aggregate 1-min to 1-min, 5-min.

## Phase 3 — Indicator(s): Rate of Change momentum threshold; fixed holding period exit
- [ ] Implement each indicator as a pure function over `&[Bar]` or `&[f64]`.
- [ ] Unit test against known reference values.

## Phase 4 — Backtest engine
- [ ] Signal logic specific to TF-19 ROC Breakout.
