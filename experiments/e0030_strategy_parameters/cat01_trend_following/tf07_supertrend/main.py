...
from indicators import build_atr_cache, ema

os.environ.setdefault("NUMBA_CACHE_DIR", str(Path(__file__).resolve().parent / ".numba_cache"))

ATR_PERIODS = [5, 7, 10, 12, 14]
MULTIPLIERS = [1.5, 2.0, 2.5, 3.0, 3.5, 4.0]
# (mode, htf_minutes, label): mode 0 = no filter.
HTF_FILTERS = [(0, 0, "none"), (1, 15, "15min"), (2, 60, "1h")]
ATR_TARGETS = [1.0, 1.5, 2.0]
TIMEFRAMES = [1, 5, 15]
HTF_TREND_PERIOD = 200
MIN_TRADES = 50
...
def _htf_trend(bars_1min: BarArrays, base_ts: np.ndarray, htf_min: int) -> np.ndarray:
    """+1/-1 trend (close vs EMA200) of the last completed htf bar, base-aligned."""
    htf = resample(bars_1min, htf_min)
    e200 = ema(htf.close, HTF_TREND_PERIOD)
    trend = np.zeros(htf.ts.size, dtype=np.int64)
    valid = ~np.isnan(e200)
    trend[valid & (htf.close > e200)] = 1
    trend[valid & (htf.close <= e200)] = -1
    bucket_idx = np.searchsorted(htf.ts, base_ts, side="right") - 1
    completed = bucket_idx - 1
    out = np.zeros(base_ts.size, dtype=np.int64)
    mask = completed >= 0
    out[mask] = trend[completed[mask]]
    return out
...
def build_grid(tf_min: int) -> list[Params]:
    ...
    for atr_p in ATR_PERIODS:
        for mult in MULTIPLIERS:
            for mode, _htf_min, label in HTF_FILTERS:
...
def _grid_arrays(grid: list[Params], atr_row: dict[int, int]) -> tuple[np.ndarray, ...]:
    n = len(grid)
    atr_rows = np.empty(n, dtype=np.int64)
    atr_p = np.empty(n, dtype=np.int64)
    mult = np.empty(n, dtype=np.float64)
    atr_target = np.empty(n, dtype=np.float64)
    htf_mode = np.empty(n, dtype=np.int64)
    for i, p in enumerate(grid):
        atr_rows[i] = atr_row[p.atr_period]
        atr_p[i] = p.atr_period
        mult[i] = p.multiplier
        atr_target[i] = p.atr_target
        htf_mode[i] = p.htf_mode
    return atr_rows, atr_p, mult, atr_target, htf_mode
...
def _prepare_tf(bars_1min: BarArrays, tf_min: int) -> TfData:
    ...
    atr_matrix, atr_row = build_atr_cache(
        bars_tf.high, bars_tf.low, bars_tf.close, ATR_PERIODS
    )
    htf_rows = [np.zeros(bars_tf.ts.size, dtype=np.int64)]
    for _mode, htf_min, _label in HTF_FILTERS[1:]:
        htf_rows.append(_htf_trend(bars_1min, bars_tf.ts, htf_min))
    htf_matrix = np.vstack(htf_rows)
    grid = build_grid(tf_min)
    atr_rows, atr_p, mult, atr_target, htf_mode = _grid_arrays(grid, atr_row)
    return TfData(
        tf_min=tf_min,
        bars=bars_tf,
        atr_matrix=atr_matrix,
        atr_row=atr_row,
        htf_matrix=htf_matrix,
        grid=grid,
        atr_rows=atr_rows,
        atr_p=atr_p,
        mult=mult,
...
def _eval_single(tf: TfData, params: Params, start_ts: int, end_ts: int) -> Metrics:
    ...
    atr_rows = np.array([tf.atr_row[params.atr_period]], dtype=np.int64)
    atr_p = np.array([params.atr_period], dtype=np.int64)
    mult = np.array([params.multiplier], dtype=np.float64)
    atr_target = np.array([params.atr_target], dtype=np.float64)
    htf_mode = np.array([params.htf_mode], dtype=np.int64)
...
def main() -> None:
    ...
    with results_path.open("w", encoding="utf-8") as fh:
        fh.write(
            "atr_period,multiplier,htf_filter,atr_target,tf_min,"
            "sharpe,profit_factor,max_drawdown_r,total_return,n_trades\n"
        )
        for p, m in all_results:
            fh.write(
                f"{p.atr_period},{p.multiplier:.1f},{p.htf_label},"
                f"{p.atr_target:.1f},{p.tf_min},"
    ...
    print(
        f"{'atrP':<5} {'mult':<5} {'htf':<6} {'tgt':<5} {'tf':<5}  "
        f"{'sharpe':>8}  {'pf':>8}  {'max_dd_r':>8}  {'tot_R':>8}  {'trades':>7}"
    )
    top10 = []
    for p, m in valid[:10]:
        print(
            f"{p.atr_period:<5} {p.multiplier:<5.1f} {p.htf_label:<6} "
            f"{p.atr_target:<5.1f} {p.tf_min:<5}  "
    ...
    with wf_path.open("w", encoding="utf-8") as wf:
        wf.write(
            ...
            "sel_atr_period,sel_multiplier,sel_htf,sel_atr_target,sel_tf,"
        ...
        for idx, (is_s, is_e, oos_s, oos_e) in enumerate(WALK_FORWARD_WINDOWS):
            ...
            selected_label = (
                f"atr{sel_params.atr_period}×{sel_params.multiplier:.1f} "
                f"htf={sel_params.htf_label} t{sel_params.atr_target:.1f} "
            ...
            wf.write(
                ...
                f"{sel_params.atr_period},{sel_params.multiplier:.1f},"
                f"{sel_params.htf_label},{sel_params.atr_target:.1f},{sel_params.tf_min},"
