"""Data access layer for tf01_ema_crossover_python.
...
Timestamp policy matches the Rust sibling: naive ``YYYY.MM.DD HH:MM:SS`` labels
are converted to epoch seconds *as if UTC* (no real EET offset), so walk-forward
year boundaries align with the Rust ``Bar::ts`` convention.
"""
...
# experiments/.../tf01_ema_crossover_python/data.py -> repo root is four levels up.
...
def year_start_ts(year: int) -> int:
    """Epoch seconds of ``year``-01-01 00:00:00 in the exchange clock."""
...
def _parse_epoch_seconds(labels: pl.Series) -> np.ndarray:
    """Convert naive timestamp strings to epoch seconds (treated as UTC)."""
    # Polars parses the wall-clock label; epoch uses the same naive-as-UTC policy
    # as the Rust chrono ``and_utc()`` conversion.
    return (
